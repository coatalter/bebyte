...
try {
    ...
    console.warn("Resetting stock data...", err);
...
window.changeReportPage = (delta) => { const data = getReport(); const totalPages = Math.ceil(data.totalTrx / itemsPerPage); const newPage = reportPage + delta; if(newPage >= 1 && newPage <= totalPages) { reportPage = newPage; renderReportTable(); } };
function renderReportTable() { const data = getReport(); const history = data.history.sort((a,b) => b.id - a.id); let currentData; let paginationControls = ''; if (isPrintingMode) { currentData = history; paginationControls = ''; } else { const totalPages = Math.ceil(history.length / itemsPerPage); if (history.length > 0 && reportPage > totalPages) reportPage = 1; const startIndex = (reportPage - 1) * itemsPerPage; const endIndex = startIndex + itemsPerPage; currentData = history.slice(startIndex, endIndex); paginationControls = `<div class="pagination-controls flex justify-between items-center mt-4 pt-2 border-t border-gray-200"><span class="text-xs text-gray-500 font-bold">Halaman ${reportPage} dari ${totalPages || 1}</span><div class="flex gap-2"><button onclick="changeReportPage(-1)" class="px-3 py-1 border border-black rounded text-xs font-bold hover:bg-gray-200 disabled:opacity-50" ${reportPage === 1 ? 'disabled' : ''}>&lt; Prev</button><button onclick="changeReportPage(1)" class="px-3 py-1 border border-black rounded text-xs font-bold hover:bg-gray-200 disabled:opacity-50" ${reportPage >= totalPages ? 'disabled' : ''}>Next &gt;</button></div></div>`; } const headerHtml = `<div class="mb-4"><h2 class="font-bold text-3xl mb-1">Riwayat Transaksi</h2><div class="flex gap-4 text-sm text-gray-600"><div class="bg-green-50 px-3 py-1 rounded border border-green-200 text-green-800 font-bold">Total: ${fmt(data.totalOmset)}</div><div class="bg-blue-50 px-3 py-1 rounded border border-blue-200 text-blue-800 font-bold">${data.totalTrx} Transaksi</div></div></div>`; const tableHeader = `<thead class="bg-gray-100 text-gray-600 text-xs uppercase font-bold text-left"><tr><th class="px-4 py-3 rounded-tl-lg">Waktu</th><th class="px-4 py-3">No. ID</th><th class="px-4 py-3 w-1/3">Item (Detail)</th><th class="px-4 py-3">Metode</th><th class="px-4 py-3 rounded-tr-lg text-right">Total</th></tr></thead>`; const tableRows = currentData.map((tx, index) => { const itemsSummary = tx.items.map(i => `<div class="font-bold text-xs text-black">• ${i.qty}x ${i.name}</div>`).join(''); const rowColor = index % 2 === 0 ? 'bg-white' : 'bg-gray-50'; const methodBadge = tx.customer.method === 'QRIS' ? '<span class="text-blue-600 font-bold bg-blue-50 px-2 py-1 rounded border border-blue-100">QRIS</span>' : '<span class="text-green-600 font-bold bg-green-50 px-2 py-1 rounded border border-green-100">TUNAI</span>'; return `<tr class="${rowColor} border-b border-gray-200 hover:bg-gray-100 transition"><td class="px-4 py-3 text-xs font-medium text-gray-500 align-top whitespace-nowrap">${new Date(tx.id).toLocaleDateString('id-ID')}<br><span class="text-[10px]">${new Date(tx.id).toLocaleTimeString('id-ID')}</span></td><td class="px-4 py-3 text-xs text-bebyte-purple font-bold align-top">#${tx.id.toString().slice(-4)}</td><td class="px-4 py-3 align-top">${itemsSummary}</td><td class="px-4 py-3 text-xs align-top">${methodBadge}</td><td class="px-4 py-3 text-sm font-bold text-black text-right align-top">${fmt(tx.total)}</td></tr>`; }).join(''); els.reportContent.innerHTML = `${headerHtml}<div class="overflow-x-auto rounded-lg border border-gray-200"><table class="w-full">${tableHeader}<tbody>${tableRows || '<tr><td colspan="5" class="p-4 text-center text-gray-400">Belum ada data</td></tr>'}</tbody></table></div>${paginationControls}`; }
...
document.getElementById('btn-print-pdf').addEventListener('click', () => { isPrintingMode = true; renderReportTable(); setTimeout(() => { window.print(); isPrintingMode = false; renderReportTable(); }, 500); });

// HANDLER TOMBOL RESET YANG BENAR
if(els.btnResetDB) {
    els.btnResetDB.addEventListener('click', () => {
        showConfirm("RESET DATABASE?", "Semua data penjualan bakal ilang permanen, yakin?", () => {
            clearReportData();
        });
    });
}
...
// Backup jika user masih pakai HTML lama (opsional, tapi aman)
if(document.getElementById('clear-report')) {
    document.getElementById('clear-report').addEventListener('click', clearReportData);
}
